...
class WarehouseApp {
    ...
    for (let i = 1; i <= numBlocks; i++) {
      const blockHtml = `
                <div class="collapsible-section">
                    <div class="section-header"><h3><i class="fas fa-cubes"></i> ${
                      i + 2
                    }. Block ${i} Information</h3><i class="fas fa-chevron-down"></i></div>
  ...
  renderRackGaps(blockIndex, numRacks) {
    ...
    let html =
      '<div class="dimension-label" style="margin-top:10px; margin-bottom:5px;"><span>Gaps Between Specific Racks (cm)</span></div><div class="dimension-row" style="flex-direction: column; flex-wrap: nowrap; gap: 10px;">';
    for (let i = 1; i < numRacks; i++) {
      html += `
                <div class="dimension-button-group" style="flex: 0 0 45%; min-width: 120px;">
                    <div class="dimension-label" style="font-size: 11px;"><span>Gap Rack ${i}-${
        i + 1
      }</span></div>
                    <div class="dimension-input-button">
                        <input type="number" class="b-custom-gap" data-gap-index="${
                          i - 1
                        }" value="50">
  ...
  initCollapsibleSections() {
    ...
    if (
      sections.length > 0 &&
      !document.querySelector(".collapsible-section.active")
    ) {
  ...
  initDimensionButtons() {
    const inputs = document.querySelectorAll(
      ".dimension-input-button input, .number-input-button input, .type-select-button select"
    );
    inputs.forEach((input) => {
      ...
      input.addEventListener("focus", () => {
        const parent = input.closest(
          ".dimension-input-button, .number-input-button, .type-select-button"
        );
        if (parent) {
  ...
  initializeEventListeners() {
    document
      .getElementById("generate-layout-btn")
      .addEventListener("click", () => this.createWarehouse());
    document
      .getElementById("reset-all-btn")
      .addEventListener("click", () => this.resetWarehouse());
    document
    ...
    document
      .getElementById("interaction-mode")
  ...
  getWarehouseConfig() {
    const getUnit = (elementId) => {
      ...
      if (element) {
        const unitSelect = element
          .closest(".dimension-input-button")
    ...
    const blockConfigs = [];
    const blockSections = document.querySelectorAll(
      "#dynamic-blocks-container .section-content"
    );

    blockSections.forEach((section, index) => {
      const getVal = (cls) =>
        parseFloat(section.querySelector(`.${cls}`).value) || 0;
      const getStr = (cls) => section.querySelector(`.${cls}`).value;
      const getLocalUnit = (cls) =>
        section
          .querySelector(`.${cls}`)
          .parentNode.querySelector(".unit-select")?.value || "cm";
  ...
  resetWarehouse() {
    if (confirm("Reset all data?")) {
      // Use the correct method name defined in ThreeJSVisualizer class
      if (window.visualizer) window.visualizer.clearWarehouse(); 
      this.currentWarehouse = null;
